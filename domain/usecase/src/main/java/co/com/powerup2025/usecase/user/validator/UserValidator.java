package co.com.powerup2025.usecase.user.validator;
...
public class UserValidator {
    ...
    public static Mono<User> validar(User user) {
        return Flux.just(
                Tuples.of(isBlank(user.getNombre()), ErrorCode.VAL_001),
                Tuples.of(isBlank(user.getApellido()), ErrorCode.VAL_002),
                Tuples.of(isBlank(user.getEmail()), ErrorCode.VAL_003),
                Tuples.of(!emailPattern.matcher(user.getEmail()).matches(),
                        ErrorCode.VAL_009),
                Tuples.of(user.getIdRol() == null, ErrorCode.VAL_007),
                Tuples.of(isSalarioInvalido(user.getSalarioBase()), ErrorCode.VAL_008))
                .filter(Tuple2::getT1)
                .map(Tuple2::getT2)
                .collectList()
